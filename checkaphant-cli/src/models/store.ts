...
export const refreshStoreKeyVotes = db.transaction((keyVotes: KeyVote[]) => {
    const stmt_clear = db.prepare(`
...
export const upsertStoreKeyVotes = db.transaction((keyVotes: KeyVote[]) => {
    ...
    for (const i in keyVotes) {
        stmt_clear.run(keyVotes[i].kid, keyVotes[i].sid);
        stmt_insert.run(
            keyVotes[i].ts,
            keyVotes[i].kid,
            keyVotes[i].type,
...
export const refreshStoreAssetVotes = db.transaction((assetVotes: AssetVote[]) => {
    const stmt_clear = db.prepare(`
...
export const upsertStoreAssetVotes = db.transaction((assetVotes: AssetVote[]) => {
    ...
    for (const i in assetVotes) {
        stmt_clear.run(assetVotes[i].uri, assetVotes[i].model, assetVotes[i].sid);
        stmt_insert.run(
            assetVotes[i].ts, 
            assetVotes[i].uri, 
            assetVotes[i].format, 
